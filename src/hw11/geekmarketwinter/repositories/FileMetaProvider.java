...
public class FileMetaProvider implements IFileMetaProvider {
    ...
    private static final String GET_FILES_META = "select hash, filename from info_metadata where sub_type = :subtype";

    private static final String GET_FILE_PATH_BY_HASH = "select filename from info_metadata where hash = :hash";

    private static final String SAVE_FILE_META_DATA = "insert into info_metadata (hash, filename, sub_type)\n" +
            "values (:hash, :finame, :subtype)";


    private static final String DEL_FILE_META = "delete from info_metadata (hash, filename, sub_type)\n" +
            "values (:hash, :finame, :subtype)";
    ...
    @Override
    public void deleteFileMeta(UUID fileHash, String fileName, int sybType) {
        if (getMetaFiles(fileHash)==1) {
            try (Connection connection = sql2o.open()) {
                connection.createQuery(DEL_FILE_META)
                        .addParameter("hash", fileHash)
                        .addParameter("finame", fileName)
                        .addParameter("subtype", sybType)
                        .executeUpdate();
    ...
    Collection<FileMetaDTO> getlistMetaFiles(UUID fileHash){
        try (Connection connection = sql2o.open()) {
            return connection.createQuery(GET_FILE_PATH_BY_HASH, false)
                    .addParameter("hash", fileHash)
                    .executeAndFetch(FileMetaDTO.class);
    ...
    @Override
    public int getMetaFiles(UUID fileHash) {
        int count=0;
        ArrayList listFiles=new ArrayList(getlistMetaFiles(fileHash));
